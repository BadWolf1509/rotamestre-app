...
| 8   | **Sentry parado em 10.69: os PRs #392 (`@sentry/react`) e #395 (`@sentry/browser`) seguem abertos.** O limite de bundle já subiu para 4 MB (PR #403) justamente para acomodá-los, mas em 17/08 o Dependabot **não processou o `@dependabot rebase`** — os dois continuam com base em `c6f7c21` e o check vermelho do #395 é o run antigo, rodado contra o teto de 3.5 MB. Basta o rebase acontecer para os dois ficarem verdes. **Mergeie os dois juntos:** são dependência acoplada (ver 04/08 no HISTORICO); separá-los deixa as versões dessincronizadas. | qualquer sessão (após o rebase)                | PRs #392 e #395                                        |
...
bloqueava era estarem no mesmo PR. O limite de bundle subiu de 3.5 MB para
**4 MB** (PR #403) para acomodar o Sentry — ver pendência 8.
