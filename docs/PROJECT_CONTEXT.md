...
| 8   | **Sentry parado em 10.69: PRs #392 (`@sentry/react`) e #395 (`@sentry/browser`) abertos. Mergeie os dois JUNTOS.** `@sentry/react@10.69.0` depende de `@sentry/browser` **exatamente 10.69.0** (versão fixa, não range): subir só um faz o npm criar **uma segunda cópia do Sentry inteiro** aninhada, e foi isso — não o tamanho do 10.70 — que estourou o bundle em 338 kB no #395 (o lock dele adiciona `@sentry/react/node_modules/@sentry/{browser,replay,feedback}` e várias cópias de `@sentry/core`; o #392 sozinho não aninha nada). **Juntos, as versões alinham e o npm deduplica.** O check vermelho do #395 é o run antigo, contra o teto de 3.5 MB — não é problema real. Em 17/08 o Dependabot **não processou o `@dependabot rebase`**; ele reclamou das labels `dependencies` e `npm`, que não existem no repo, e essa é a pista mais provável do travamento. | qualquer sessão (após o rebase)                | PRs #392 e #395                                        |
...
O limite de bundle subiu de 3.5 MB para **4 MB** (PR #403), e vale saber com que
fundamento: na hora, os 338 kB a mais do #395 foram lidos como crescimento real
do Sentry 10.70. **Não eram** — a apuração posterior do `package-lock` mostrou
que era duplicação por versões dessincronizadas (ver pendência 8). Com os dois
PRs juntos o bundle deve voltar ao patamar anterior. O teto de 4 MB continua
sendo uma folga defensável, mas **se o Bundle Size confirmar a volta ao normal,
considere devolvê-lo a 3.5 MB** — um alarme mais apertado avisa mais cedo.
