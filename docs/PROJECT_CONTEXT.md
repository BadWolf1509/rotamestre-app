...
- **9.** **Três decisões de produto levantadas no smoke de 05/09/2026 — nenhuma é defeito.** (a) **Localização é só de primeiro plano e nenhuma posição do motorista é persistida** — não existe tabela; com o app em background o registro de GPS vira `ProviderRequest[OFF]`. O "tempo real" que o gestor vê é de **status**, e funciona. Como o material fala em rastreamento em tempo real, vale alinhar discurso e produto antes da loja. (b) **"Excluir motorista" é só desativar**, e o vínculo em `usuario_unidades` continua `ativo` — o seletor do app checa os dois, mas código que filtre só pelo vínculo veria um motorista desativado. (c) **Não existe caminho para reabrir parada concluída** em nenhuma das duas contas: o botão "Retomar" só aparece para parada _pulada_, e o painel do gestor não oferece a ação.<br>_Quem:_ gestor (decisão) · _Detalhe:_ [HISTORICO.md](HISTORICO.md), sessão de 05/09
