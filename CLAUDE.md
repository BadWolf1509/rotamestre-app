...
**Last verified:** 2026-07-15 (Expo 56, RN 0.85.3, ~5747 tests / ~74% coverage; app nativo **v1.12.1 / versionCode 3020**. **C3 Fase 1+2 em produção** — bucket privado + signed URLs + RLS por unidade, PRs #285/#294. **Fix reordenar paradas (PR #312, 15/07)** — modal não salvava em rota com parada `em_andamento` (status órfão no split → colisão UNIQUE no `reordenar_paradas`); só frontend, sem mudança de banco (ver phonebook "UI de reordenação"). **Migration 15 (PR #310, 13/07)** — fix `inserir_parada`: inserir parada no meio violava UNIQUE(rota_id, ordem) com a chegada (`database/MIGRATIONS.md`). Deps 13/07 (#306–#309/#298): `supabase-js` 2.110.3, `toast-message` 2.4.0, eslint 9.39.5, e o **par acoplado** `unistyles` 3.3.0 + `nitro-modules` 0.36.1 — unistyles é o único consumidor de nitro e as versões devem andar juntas (mismatch só aparece em **build EAS**, o CI é web/Jest). `npm audit` prod sem high/critical. App rebuilt under new package/EAS/Firebase — see memory. Histórico de fixes anteriores: memória `improvements-roadmap-2026-06`)
