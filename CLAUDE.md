...
| Cadastro → onboarding (sem perfil ainda)   | `signUp` cria **só** a conta no Auth (`src/lib/auth.ts`); o perfil e a unidade nascem depois, na RPC `criar_unidade_para_novo_gestor` (`src/hooks/onboarding/useCriarUnidade.ts`). Entre um e outro, `usuarios` **não tem linha** — `user_metadata` é a única fonte de dados da pessoa, e consulta com `.single()` ali devolve `PGRST116`, que é estado esperado, não erro                                                           |
