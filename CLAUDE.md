...
- **Mapa web (maplibre 6):** o v6 é ESM-only e resolve seu web worker por `import.meta.url`, que o Metro **não** empacota. O worker é servido de `public/` (copiado de `node_modules` por `tools/scripts/copy-maplibre-worker.cjs` no `postinstall` e no `build:web`) e apontado por `configureMaplibreWorker()` (`src/lib/maplibreWorker.ts`), chamado nos 5 componentes de mapa web. **Quebrar isso trava o mapa em "Carregando..." sem erro no console** — nenhum teste de _screenshot_ detecta, porque os que existiam mascaravam a região do mapa. Desde 25/08/2026 o e2e `renders motorista mapa` cobre isso funcionalmente (o testID `mapa-web-carregando` só some no evento `load`) e roda em CI. **O job de visual regression precisa copiar o worker explicitamente**: instala com `npm ci --ignore-scripts`, que pula o `postinstall`, e não roda `build:web`. O nativo usa `@maplibre/maplibre-react-native` e não é afetado.
