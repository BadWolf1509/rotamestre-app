...
const mockParadas = [
  { rota_id: 'rota-1', status: 'pendente', is_checkpoint: true, rotas: { unidade_id: 'unidade-1' } },
  { rota_id: 'rota-1', status: 'pendente', is_checkpoint: true, rotas: { unidade_id: 'unidade-1' } },
  { rota_id: 'rota-2', status: 'concluida', is_checkpoint: true, rotas: { unidade_id: 'unidade-1' } },
  { rota_id: 'rota-2', status: 'pendente', is_checkpoint: true, rotas: { unidade_id: 'unidade-1' } },
  { rota_id: 'rota-3', status: 'concluida', is_checkpoint: true, rotas: { unidade_id: 'unidade-1' } },
];
  ...
  const mockRotasQuery = {
    ...
    abortSignal: jest.fn().mockResolvedValue({
      data: rotasData?.map(r => ({
        ...r,
        usuarios: r.motorista_id ? { id: r.motorista_id, nome: r.motorista_nome } : null,
      })),
      error: rotasError,
    }),
  };
...
describe('useGestaoRotas', () => {
  ...
  describe('Data Loading', () => {
    ...
    it('deve tratar erro de carregamento sem cache', async () => {
      ...
      await waitFor(() => {
        expect(Alert.alert).toHaveBeenCalledWith('Erro', 'Não foi possível carregar as rotas');
      });
    });
  ...
  describe('Filtragem', () => {
    it('deve filtrar por status pendente', async () => {
      ...
      await waitFor(() => {
        expect(result.current.rotasFiltradas.every(r => r.status === 'pendente')).toBe(true);
      });
    });
    ...
    it('deve filtrar por status em_andamento', async () => {
      ...
      await waitFor(() => {
        expect(result.current.rotasFiltradas.every(r => r.status === 'em_andamento')).toBe(true);
      });
    });
    ...
    it('deve mostrar todas as rotas quando filtro é "todas"', async () => {
      ...
      await waitFor(() => {
        expect(result.current.rotasFiltradas.length).toBe(result.current.rotas.length);
      });
    });
    ...
    it('deve filtrar por busca de nome do motorista', async () => {
      ...
      await waitFor(() => {
        expect(result.current.rotasFiltradas.every(r =>
  ...
  describe('Exclusão de Rota', () => {
    it('deve rejeitar exclusão de rota em andamento (mobile)', async () => {
      ...
      const rotaEmAndamento = result.current.rotas.find(r => r.status === 'em_andamento')!;
      ...
      (mockSupabase.from as jest.Mock).mockImplementation((table: string) => {
        ...
        if (table === 'paradas') {
          return {
            select: jest.fn().mockReturnThis(),
            eq: jest.fn().mockReturnThis(),
            abortSignal: jest.fn().mockResolvedValue({ data: mockParadas, error: null }),
          };
        }
    ...
    it('deve mostrar confirmação no mobile (Alert.alert)', async () => {
      ...
      const rotaPendente = result.current.rotas.find(r => r.status === 'pendente')!;
  ...
  describe('Exportação CSV', () => {
    it('deve alertar quando não há rotas para exportar', async () => {
      ...
      expect(Alert.alert).toHaveBeenCalledWith('Atenção', 'Não há rotas para exportar');
    });
  ...
  describe('Helpers', () => {
    it('deve retornar label correto para status', async () => {
      ...
      expect(result.current.getStatusLabel('em_andamento')).toBe('Em Andamento');
      ...
      expect(result.current.getStatusLabel('nao_executada')).toBe('Não Executada');
    });
