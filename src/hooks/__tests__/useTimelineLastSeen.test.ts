...
import { useTimelineLastSeen, clearTimelineLastSeen, clearAllTimelineLastSeen } from '../useTimelineLastSeen';
...
describe('useTimelineLastSeen', () => {
  ...
  it('carrega lastSeen do storage', async () => {
    (AsyncStorage.getItem as jest.Mock).mockResolvedValue('2024-01-01T00:00:00.000Z');
  ...
  it('markAsSeen ignora timestamp mais antigo', async () => {
    (AsyncStorage.getItem as jest.Mock).mockResolvedValue('2024-01-01T02:00:00.000Z');
...
describe('clearTimelineLastSeen', () => {
  ...
  it('remove chave da rota', async () => {
    ...
    expect(AsyncStorage.removeItem).toHaveBeenCalledWith('@timeline_last_seen_rota-1');
  });
