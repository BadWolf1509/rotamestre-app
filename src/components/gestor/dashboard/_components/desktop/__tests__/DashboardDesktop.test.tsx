            ...
            <View testID="rotas-table">
                {rotas.map((rota: any) => (
                    <View key={rota.id} testID={`rota-row-${rota.id}`}>
                        ...
                        <TouchableOpacity testID={`view-${rota.id}`} onPress={() => onView?.(rota.id)}>
                            <Text>Ver</Text>
                        </TouchableOpacity>
                        <TouchableOpacity testID={`delete-${rota.id}`} onPress={() => onDelete?.(rota.id)}>
                            <Text>Excluir</Text>
                        </TouchableOpacity>
                    </View>
                ))}
...
describe('DashboardDesktop', () => {
    const defaultProps = {
        ...
        rotas: [
            { id: 'rota-1', motorista_nome: 'João Silva', status: 'em_andamento', total_paradas: 5, paradas_concluidas: 2 },
            { id: 'rota-2', motorista_nome: 'Maria Santos', status: 'pendente', total_paradas: 3, paradas_concluidas: 0 },
        ],
        loading: false,
        refreshing: false,
        onRefresh: jest.fn(),
