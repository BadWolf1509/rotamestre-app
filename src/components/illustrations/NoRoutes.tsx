...
export const NoRoutes = memo(function NoRoutes({ width = 160, height = 140 }: IllustrationProps) {
  const { theme } = useUnistyles();
  const bg = withOpacity(theme.colors.primary, 0.1);
  return (
    <View style={[styles.root, { width, height }]} accessibilityLabel="Nenhuma rota">
      ...
      <View style={styles.markerLeft}>
        ...
        <View style={[styles.stem, { backgroundColor: theme.colors.primary }]} />
      </View>
      <View style={styles.markerRight}>
        <View style={[styles.dot, { backgroundColor: theme.colors.secondary }]} />
        <View style={[styles.stem, { backgroundColor: theme.colors.secondary }]} />
      </View>
    </View>
  );
});
...
const styles = StyleSheet.create((_theme: Theme) => ({
  ...
  road: { width: 120, height: 4, borderWidth: 2, borderStyle: 'dashed', borderRadius: 2 },
  markerLeft: { position: 'absolute', left: 28, top: 38, alignItems: 'center' },
  markerRight: { position: 'absolute', right: 28, top: 38, alignItems: 'center' },
