...
describe('PictureInPictureMap', () => {
  ...
  describe('Rendering', () => {
    ...
    it('deve renderizar apenas com userLocation', () => {
      const { getByTestId } = render(
        <PictureInPictureMap {...defaultProps} destination={null} />,
      );
    ...
    it('deve renderizar marker quando há destination', () => {
      const { getAllByTestId } = render(
        <PictureInPictureMap {...defaultProps} />,
      );
    ...
    it('não deve renderizar marker quando destination é null', () => {
      ...
      const destinationMarker = markers.find(
        (marker) =>
          marker.props.lngLat?.[0] === -46.64 &&
          marker.props.lngLat?.[1] === -23.56,
      );
  ...
  describe('Region calculation', () => {
    it('deve calcular região corretamente com userLocation e destination', () => {
      ...
      expect(camera.props.center).toBeDefined();
    });
    ...
    it('deve calcular centro da região entre userLocation e destination', () => {
      ...
      const camera = getByTestId('map-camera');
      const center = camera.props.center;

      // Centro deve estar entre userLocation e destination
      expect(center[1]).toBeGreaterThanOrEqual(-23.56);
    ...
    it('deve usar userLocation como região quando não há destination', () => {
      ...
      const camera = getByTestId('map-camera');
      expect(camera.props.center).toEqual([-46.6333, -23.5505]);
    });
  });
  ...
  describe('Map properties', () => {
    ...
    it('deve desabilitar interações quando colapsado', () => {
      ...
      const mapView = getByTestId('pip-map-view');
      expect(mapView.props.dragPan).toBe(false);
      expect(mapView.props.touchZoom).toBe(false);
      expect(mapView.props.compass).toBe(false);
    });
  });
  ...
  describe('Edge cases', () => {
    ...
    it('deve lidar com destination ao norte do userLocation', () => {
      const props = {
        ...defaultProps,
        userLocation: { latitude: -23.56, longitude: -46.6333 },
        destination: {
          latitude: -23.54,
          longitude: -46.6333,
          address: 'Norte',
    ...
    it('deve lidar com destination ao sul do userLocation', () => {
      const props = {
        ...defaultProps,
        userLocation: { latitude: -23.54, longitude: -46.6333 },
        destination: { latitude: -23.56, longitude: -46.6333, address: 'Sul' },
      };
