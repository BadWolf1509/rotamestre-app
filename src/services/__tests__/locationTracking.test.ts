...
describe('LocationTrackingService', () => {
    ...
    describe('startTracking', () => {
        const stopData = { latitude: -23.55, longitude: -46.63, endereco: 'Rua Test 123' };

        it('deve iniciar tracking com sucesso', async () => {
            const chain = createChain({ data: stopData, error: null });
            mockFrom.mockReturnValue(chain);

            const result = await locationTrackingService.startTracking('rota-1', 'stop-1', 'stop-2');
        ...
        it('deve retornar false quando foreground permission negada', async () => {
            ...
            const result = await locationTrackingService.startTracking('rota-1', 'stop-1');
    ...
    describe('stopTracking', () => {
        it('deve parar task e limpar estado quando task está rodando', async () => {
            ...
            (locationTrackingService as any).navigationState = { enabled: true, rotaId: 'r1' };
            (locationTrackingService as any).arrivalTimeout = setTimeout(() => {}, 99999);
    ...
    describe('isTracking', () => {
        it('deve delegar para Location.hasStartedLocationUpdatesAsync', async () => {
            ...
            expect(result).toBe(true);
            expect(Location.hasStartedLocationUpdatesAsync).toHaveBeenCalledWith('background-location-tracking');
        });
    });
    ...
    describe('processLocationUpdate', () => {
        ...
        it('deve atualizar posição do motorista no banco', async () => {
            ...
            const farLocation = { ...baseLocation, latitude: -23.56, longitude: -46.64 };
    ...
    describe('handleProximityNotifications via processLocationUpdate', () => {
        ...
        it('deve atualizar lastNotificationTime para distância < 100m', async () => {
            ...
            expect((locationTrackingService as any).lastNotificationTime).toBeGreaterThan(0);
        });
        ...
        it('deve respeitar throttle de 30 segundos', async () => {
            ...
            expect((locationTrackingService as any).lastNotificationTime).toBe(timeBefore);
        });
    });
});
