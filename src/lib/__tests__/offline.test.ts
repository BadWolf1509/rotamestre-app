...
const FileSystem = require('expo-file-system/legacy') as Record<
  string,
  jest.Mock
>;

const { uploadELinkFotoParada } = require('../storage') as {
  uploadELinkFotoParada: jest.Mock;
};
...
describe('offline', () => {
  ...
  describe('processOfflineQueue', () => {
    ...
    /**
     * A Migration 27 revogou `UPDATE (unidade_id)` de `rotas`. Sem
     * allowlist, um payload que carregue essa coluna faz o Postgres
     * derrubar o UPDATE INTEIRO — `status` e `concluida_em` junto —, a ação
     * reenfileira, e a entrega fica registrada em lugar nenhum.
     *
     * Não há produtor hoje (`addToOfflineQueue` não tem chamador), mas
     * `setupOfflineSync` drena a fila a cada reconexão: o primeiro produtor
     * que aparecer passa por aqui.
     */
    it('finalizar_rota descarta coluna que o REVOKE da Migration 27 derrubaria', async () => {
      mockNetInfo.fetch.mockResolvedValueOnce({
        isConnected: true,
        isInternetReachable: true,
      } as any);
      ...
      (supabase.from as jest.Mock).mockReturnValueOnce({ update } as any);
      ...
      const queue = [
        {
          ...
          data: {
            rotaId: 'r1',
            status: 'concluida',
            concluida_em: '2026-09-08T00:00:00Z',
            unidade_id: 'u1',
            motorista_id: 'm1',
          },
  ...
  describe('removeFromPhotosIndex', () => {
    it('should remove only the specific photo entry by localPath, not all for same paradaId', async () => {
      const photos = [
        {
          localPath: '/path/parada-1_111.jpg',
          paradaId: 'parada-1',
          unidadeId: 'u1',
          rotaId: 'r1',
          originalUri: 'file:///p1a.jpg',
          savedAt: '2026-01-01T00:00:00.000Z',
        },
        {
          localPath: '/path/parada-1_222.jpg',
          paradaId: 'parada-1',
          unidadeId: 'u1',
          rotaId: 'r1',
          originalUri: 'file:///p1b.jpg',
          savedAt: '2026-01-01T00:01:00.000Z',
        },
        {
          localPath: '/path/parada-2_333.jpg',
          paradaId: 'parada-2',
          unidadeId: 'u1',
          rotaId: 'r1',
          originalUri: 'file:///p2.jpg',
          savedAt: '2026-01-01T00:02:00.000Z',
        },
      ];
      ...
      uploadELinkFotoParada.mockResolvedValueOnce(
      ...
      mockAsyncStorage.getItem.mockResolvedValueOnce(
        JSON.stringify(afterFirstRemoval),
      );
      mockAsyncStorage.setItem.mockResolvedValueOnce(undefined as any);
      ...
      mockAsyncStorage.getItem.mockResolvedValueOnce(
        JSON.stringify(afterSecondRemoval),
      );
      mockAsyncStorage.setItem.mockResolvedValueOnce(undefined as any);
  ...
  describe('hasOfflinePhoto', () => {
    ...
    it('deve retornar false quando parada não tem foto offline', async () => {
      const photos = [{ localPath: '/path/1.jpg', paradaId: 'p1' }];
      mockAsyncStorage.getItem.mockResolvedValueOnce(JSON.stringify(photos));
  ...
  describe('getOfflinePhotoPath', () => {
    ...
    it('deve retornar caminho da foto quando existe', async () => {
      const photos = [{ localPath: '/path/to/photo.jpg', paradaId: 'p1' }];
      mockAsyncStorage.getItem.mockResolvedValueOnce(JSON.stringify(photos));
  ...
  describe('queuePhotoUpload', () => {
    ...
    it('deve adicionar foto ao photos index com dados corretos', async () => {
      ...
      expect(savedData[0].localPath).toContain(
        '/mock/documents/offline_photos/',
      );
      expect(savedData[0].savedAt).toBeDefined();
    });

    it('deve gerar nomes de arquivo únicos por parada', async () => {
      const result1 = await queuePhotoUpload(
        'u1',
        'r1',
        'parada-A',
        'file:///photo1.jpg',
      );
      const result2 = await queuePhotoUpload(
        'u1',
        'r1',
        'parada-B',
        'file:///photo2.jpg',
    ...
    it('deve funcionar com photos index existente (append)', async () => {
      ...
      const existingPhotos = [
        {
          localPath: '/mock/path/existing.jpg',
          paradaId: 'p-existing',
          unidadeId: 'u1',
          rotaId: 'r1',
          originalUri: 'file:///existing.jpg',
          savedAt: '2026-01-01T00:00:00.000Z',
        },
      ];
      mockAsyncStorage.getItem.mockResolvedValueOnce(
