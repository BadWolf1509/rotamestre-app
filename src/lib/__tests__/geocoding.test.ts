...
jest.mock('@/lib/viacep', () => ({
  viacepService: {
    ...
    extractNumberFromCEPInput: (...args: unknown[]) => mockViacepExtractNumberFromCEPInput(...args),
    isCEP: jest.fn(),
    formatCEP: jest.fn(),
  },
}));
...
jest.mock('@/lib/googlePlaces', () => ({
  googlePlacesService: {
    ...
    autocompleteAddress: (...args: unknown[]) => mockGoogleAutocompleteAddress(...args),
    getPlaceDetails: (...args: unknown[]) => mockGoogleGetPlaceDetails(...args),
    autocompleteWithCoordinates: (...args: unknown[]) => mockGoogleAutocompleteWithCoordinates(...args),
  },
}));
...
describe('geocodingService', () => {
  ...
  describe('autocomplete', () => {
    ...
    it('should fall through to Google when CEP is not found', async () => {
      ...
      expect(mockGoogleAutocompleteAddress).toHaveBeenCalledWith('00000-000');
      expect(result).toHaveLength(1);
    ...
    it('should route text input to Google Places', async () => {
      ...
      expect(mockGoogleAutocompleteAddress).toHaveBeenCalledWith('Rua Antonio Leite');
      expect(result).toHaveLength(1);
      expect(result[0].source).toBe('google');
    ...
    it('should set needsCoordinates to false for Google results with coords', async () => {
      mockViacepContainsCEP.mockReturnValue(false);
      mockGoogleIsAvailable.mockReturnValue(true);
      mockGoogleAutocompleteAddress.mockResolvedValue([mockGoogleSuggestionWithCoords]);

      const result = await geocodingService.autocomplete('Rua Antonio Leite');

      expect(result[0].needsCoordinates).toBe(false);
      expect(result[0].coordinates).toEqual({ latitude: -23.5505, longitude: -46.6333 });
    });
    ...
    it('should skip CEP detection when enableCEPDetection is false', async () => {
      ...
      expect(mockViacepContainsCEP).not.toHaveBeenCalled();
      expect(mockGoogleAutocompleteAddress).toHaveBeenCalledWith('58068-504');
    });
  });
  ...
  describe('getCoordinates', () => {
    ...
    it('should fetch coordinates via Photon for viacep source', async () => {
      ...
      expect(mockPhotonGetCoordinates).toHaveBeenCalledWith(suggestion.description);
      expect(result).toEqual(mockCoords);
    });
  });
  ...
  describe('autocompleteWithCoordinates', () => {
    it('should return null when no suggestions found', async () => {
      ...
      const result = await geocodingService.autocompleteWithCoordinates('Unknown Place XYZ');
    ...
    it('should return suggestion directly if it already has coordinates', async () => {
      ...
      const result = await geocodingService.autocompleteWithCoordinates('Rua Antonio Leite');

      expect(result).not.toBeNull();
      expect(result!.coordinates).toEqual({ latitude: -23.5505, longitude: -46.6333 });
    ...
    it('should resolve coordinates for suggestion without them', async () => {
      ...
      expect(result).not.toBeNull();
      expect(result!.coordinates).toEqual(mockEnderecoGeocodificado.coordenadas);
  ...
  describe('geocode', () => {
    ...
    it('should return geocoded address from Google', async () => {
      ...
      const result = await geocodingService.geocode('Rua Antonio Leite, 100, SP');

      expect(result).not.toBeNull();
      expect(result!.coordenadas).toEqual({ latitude: -23.5505, longitude: -46.6333 });
  ...
  describe('reverseGeocode', () => {
    it('should delegate to photonService', async () => {
      mockPhotonReverseGeocode.mockResolvedValue('Rua Teste, Centro, João Pessoa, PB');
